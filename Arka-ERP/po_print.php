<?php
  require('includes/common.php');
	...
	$ObjPoReg	= new PoReg();
	$ObjPoReg->PoPrint($_GET['supplier_code']);
	$ObjPoReg->PoMgmt($_GET['supplier_code']);
	$ObjPoReg->ToAddress($_GET['po_no']); 
	$ObjPoReg->ToAddress3($_GET['po_no']); 
	$ObjPoReg->add_Total($_GET['sales_quote_no']);
	$ObjPoReg->sub_Total($_GET['sales_quote_no']);
	$ObjPoReg->tax_Total($_GET['sales_quote_no']);
	$ObjPoReg->GetAccDetails($_GET['supplier_code']);
	$ObjPoReg->GetNumbers($_GET['sales_quote_no']);

  $objSmarty->assign('IncludeTpl', 'po_print.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- Po iNVO');
