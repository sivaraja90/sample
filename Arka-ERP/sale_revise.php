<?php
  require('includes/common.php');
	...
	{
		$objSalesQuoteReg->EditSalesRevise($_POST);
	}
	
		if(isset($_GET['sales_quote_no']) && isset($_GET['revised_version']))
	{
		$objSalesQuoteReg->Revised_List(($_GET['sales_quote_no']) && $_GET['revised_version']);
		$objSalesQuoteReg->Revis_Sale(($_GET['sales_quote_no']) && $_GET['revised_version']);
	}
	
	if(isset($_GET['sales_quote_no']))
	{
		$objSalesQuoteReg->SelectQuote(($_GET['sales_quote_no']));
	}
		if(isset($_GET['succs_msg_for_insert']))
	{
	 ...
	 if((isset($_GET['sl_no'])) && (isset($_GET['revised_version'])) && (isset($_GET['sales_quote_no'])) && (isset($_GET['total'])) && (isset($_GET['sub_total'])) && (isset($_GET['pro'])))
	{ 

		$objSalesQuoteReg->DeleteRev($_GET['sl_no'],$_GET['revised_version'], $_GET['sales_quote_no'] , $_GET['total'] , $_GET['sub_total'] , $_GET['pro']);
	
	} 
	if(isset($_GET['succs_msg_for_del']))
	 {
	  $objSmarty->assign('SuccessMessage', 'Sales Revised Deleted Successfully');
	 }
	//	$ObjDashboard->RolesLists();
	// $ObjDashboard->myCurl($objArray);

  $objSmarty->assign('IncludeTpl', 'sale_revise.tpl');
