<?php
  require('includes/common.php');
	...
	$objEnquiryReg->Enquirythrough();	
	$objEnquiryReg->GetDemoProductLists($objArray);
	$objEnquiryReg->UsernameList($objArray);
	
	if(isset($_POST['hdAction']))
			{
				$objEnquiryReg->InsertEnquiry($_POST);
			}
	if(isset($_GET['succs_msg_for_insert']))
         {
          $objSmarty->assign('SuccessMessage', 'New Enquiry Added Successfully');
         }
	if(isset($_GET['company_name']))
	{
        $objEnquiryReg->AjaxGetCustomerDetails($_GET['company_name']);		
	}
		  

  $objSmarty->assign('IncludeTpl', 'enquiry_reg.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- EnquiryReg');
