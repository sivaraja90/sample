<?php
  require('includes/common.php');
	...
	$ObjProjectOrder->mail_list($objArray);
		$objStaff->chkStaffLogin($objArray);
	$objStaff->RolesLists($objArray);
	if(isset($_GET['revised_version']))
	{
	$ObjProjectOrder->mail_list($_GET['revised_version']);
	}
		if(isset($_POST['email_id1']))
	{
		$ObjProjectOrder->sendMail($_POST['email_id1']);
	}
		if(isset($_POST['hdAction']))
	{
		$ObjProjectOrder->InsertProjectOrderMail($_POST,$_FILES);
	}
...
if(isset($_GET['succs_msg']))
 {
  ...
  $objSmarty->assign('IncludeTpl', 'project_order_mail.tpl');
