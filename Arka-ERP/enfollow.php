...
<?php
  require('includes/common.php');
	...
	include_once $config['SiteClassPath']."class.FollowUpReg.php";
	...
	$ObjFollowUpReg	= new FollowUpReg();
	if(isset($_GET['enquiry_id']))
	{
	$ObjFollowUpReg	->ViewEnquirymaster($_GET['enquiry_id']);
	$ObjFollowUpReg	->EnquiryStatus($objArray);
	}
   $ObjFollowUpReg	->GetEnquiryType($_GET['enquiry_id']);
	if(isset($_POST['hdAction']))
	{
	$ObjFollowUpReg->InsertFollowUp($_POST);
	}
	if(isset($_GET['succs_msg_for_insert']))
         {
          $objSmarty->assign('SuccessMessage', 'New FollowUp Added Successfully');
         } 

  $objSmarty->assign('IncludeTpl', 'followup.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- FollowUps');
