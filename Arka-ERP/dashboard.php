<?php
  require('includes/common.php');
	...
	$objStaff->RolesLists();
	$ObjDashboard->GetAnnualEnquiry();
	$ObjDashboard->Sales();
	$ObjDashboard->Service();
	$ObjDashboard->Project();
	$ObjDashboard->graphSales();

	// $ObjDashboard->myCurl($objArray);

  $objSmarty->assign('IncludeTpl', 'dashboard.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- Dashboard');
  ...
  $objSmarty->assign('address',$config['SiteGlobalPath']);
   ...
   '<script src="' . $config['SiteGlobalPath'] . 'plugins/chartjs/Chart.min.js"></script>'. "\n" .   
