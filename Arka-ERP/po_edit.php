<?php
  require('includes/common.php');
	...
	{
	$objPoReg->EditPO($_POST);
	}
	if(isset($_GET['po_no']))
	{
	$objPoReg->ListMast($_GET['po_no']);
	$objPoReg->ListDetails($_GET['po_no']);
	}
	if(isset($_GET['succs_msg_for_insert']))
	 {
	  $objSmarty->assign('SuccessMessage', 'PO Updated Successfully');
	 } 
	if((isset($_GET['sl_no'])) && (isset($_GET['po_no'])))
	{
	$objPoReg->DeletePODetail($_GET['sl_no'] , $_GET['po_no']);
	}
  ...
  $objSmarty->assign('IncludeTpl', 'po_edit.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- PoReg');
