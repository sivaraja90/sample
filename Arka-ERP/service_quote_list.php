<?php
  require('includes/common.php');
	...
	$ObjServiceOrder->ListServiceOrder($_GET['service_order_no']);
	
	if(isset($_GET['service_order_no']))
	{
		$ObjServiceOrder->DeleteServiceEditDetails($_GET['service_order_no']);
	}
		if(isset($_GET['succs_msg_for_del']))
         {
          $objSmarty->assign('SuccessMessage', 'Service Order Deleted Successfully');
         } 
  $objSmarty->assign('IncludeTpl', 'service_quote_list.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- Service Quote List');
