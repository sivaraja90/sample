    ...
    <div class="wrapper">
      ...
      <section class="invoice">
        ...
        <div class="row">
		 ...
		 <h5 style="margin-top:-10px; margin-right:25px;" align="right">(Duplicate Copy)</h5>
        ...
        <div class="row invoice-info">
          <div class="col-sm-4 invoice-col">
			...
			Consignee<br> 
              <strong>{$View.customer_name},</strong><br>
              {$View.address1},<br>
		    ...
		    <small class="pull-right"><address style="margin-top:-250px;">
				...
				{foreach from=$AddressView item=View}
					<table class="tablepress" style="border:solid 1px #000000; margin-left:-2%; border-bottom-color:#FFFFFF; border-left-color:#FFFFFF; border-right-color:#FFFFFF; border-top-color:#FFFFFF; width:390px; height:80px; margin-top:15px;">
						...
						<td class="b1">{$View.sales_order_no}</td>
							
						<td class="b1"  style="font-weight:bold;">Order Date:</td>
						<td class="b1">{if $View.sales_order_date eq '0000-00-00'} NA {elseif $View.sales_order_date neq '0000-00-00'}{$View.sales_order_date}{/if}</td>
						</tr>
						<tr>
						<td class="b1"  style="font-weight:bold;">Invoice No:</td>
						<td class="b1">{if $View.invoice_no eq "" } NA {elseif $View.invoice_no neq ""}{$View.invoice_no}{/if}</td>
			   ...
			   {foreach from=$MailView item=View}
                <tr id="{$i++}">
                 ...
                 <!--<td align="center"><img src="./images/product/{$View.image}" style="width:200px; height:100px;" alt="{$View.image}"></td>-->
                  <td>{$View.quantity}</td>
                  <td>{$View.unit_basic}({$AddressView3.0.vat_per}%)</td>
				  <td>{$View.total}</td>
                </tr>
               {/foreach}
              </tbody>
...
<!--				{foreach from=$Add_tax item=value}
                <tr align="right">
                  <th>Vat @ ({$AddressView3.0.vat_per}%)</th>
                  <td>{$value.tax}</td>
                </tr>
				{/foreach}-->
