<?php
  require('includes/common.php');
	...
	$ObjUserMgmt->GetUserView($objArray);
		$objStaff->chkStaffLogin($objArray);
	$objStaff->RolesLists($objArray);

if(isset($_GET['s_no']))
{ 
	$ObjUserMgmt->DeleteMaster($_GET['s_no']);
}
if(isset($_GET['succs_msg_for_del']))
{
	$objSmarty->assign('SuccessMessage', 'User Deleted Successfully');
}

	$objSmarty->assign('IncludeTpl', 'user_list.tpl');
	$objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- UserReg');
