...
<?php
  require('includes/common.php');
	...
	$ObjProjectOrder	= new ProjectOrder();
		$ObjProjectOrder->ViewPaymentInvoice();
		if(isset($_GET['project_order_no']))
	{
	$ObjProjectOrder	->ToAddress1($_GET["project_order_no"]);

	}
    $ObjProjectOrder	->GetEnquiryType($objArray);
	if(isset($_POST['hdAction']))
	{
	$ObjProjectOrder->InsertProjectMakepayment($_POST);
	}
	
	if(isset($_GET['succs_msg_for_insert']))
         {
          $objSmarty->assign('SuccessMessage', 'New Payment Added Successfully');
         } 
 	

  $objSmarty->assign('IncludeTpl', 'project_makepayment.tpl');
