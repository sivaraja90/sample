<?php
  require('includes/common.php');
	...
	{
	//$ObjProjectQuoteReg->ViewEdit($_GET['project_quote_no']);
   // $ObjProjectQuoteReg->list_project_details($_GET['project_quote_no']);
	}
	if(isset($_POST['hdAction']))
	{
	$ObjProjectQuoteReg->AddProject($_POST);
	}
	if(isset($_GET['project_quote_no']))
	{
		$ObjProjectQuoteReg->SelectId($_GET['project_quote_no']);
	}
	...
	if(isset($_GET['succs_msg_for_insert']))
	 {
	 	  $objSmarty->assign('SuccessMessage', 'Project Updated Successfully');
	 } 
	if((isset($_GET['sl_no'])) && (isset($_GET['project_quote_no'])) && (isset($_GET['revised_version'])) && (isset($_GET['total'])) && (isset($_GET['sub_total'])))
	{
	
		$ObjProjectQuoteReg->DeleteProjectEdit($_GET['sl_no'],$_GET['project_quote_no'], $_GET['revised_version'],$_GET['total'],$_GET['sub_total']);
	}
  ...
  $objSmarty->assign('IncludeTpl', 'project_quote_edit.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- SalesQuote edit');
