...
{
	$objSmarty->assign('SuccessMessage', 'Update Prospect and Entitled Added Successfully');
} 
if(isset($_POST['hdAction']))
{
	$Objprospect->Editprospect($_POST);
}
if(isset($_GET['prospect_id']))
{
	$Objprospect->Viewprospect($_GET['prospect_id']);
}
if(isset($_GET['succs_msg_for_insert']))
{
	$objSmarty->assign('SuccessMessage', 'Prospect Updated Successfully');
}
	
	$objSmarty->assign('IncludeTpl', 'prospect_edit.tpl');
	$objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- Prospect');
