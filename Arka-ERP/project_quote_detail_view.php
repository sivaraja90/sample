<?php
  require('includes/common.php');
	...
	if(isset($_GET['project_order_no']))
	{
		$ObjProjectOrder->ViewProjectOrderQuote($_GET['project_order_no']);
		$ObjProjectOrder->ViewProjectDetails($_GET['project_order_no']);
		$ObjProjectOrder->ViewProjectOrderFollowupList($_GET['project_order_no']);
		$ObjProjectOrder->ViewInvoice($_GET['project_order_no']);
	    $ObjProjectOrder->ViewPayment($_GET['project_order_no']);
	}
	if((isset($_GET['sl_no'])) && (isset($_GET['project_order_no'])))
	{
	$ObjProjectOrder->DeleteOrder($_GET['sl_no'] , $_GET['project_order_no']);
	}
     if(isset($_GET['succs_msg_for_del']))
         {
          $objSmarty->assign('SuccessMessage', 'Project Order FollowUp Deleted Successfully');
         } 
    $ObjProjectOrder->DateStamp();
	$ObjProjectOrder->TimeStamp();
	$ObjProjectOrder->created_date();
	
  $objSmarty->assign('IncludeTpl', 'project_quote_detail_view.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- ProductDetail');
