<?php
  require('includes/common.php');
	...
	{
	$ObjQuoteReg->EditQuote($_POST);
	}
	if(isset($_GET['quote_id']))
	{
	$ObjQuoteReg->ListMast($_GET['quote_id']);
	$ObjQuoteReg->ListDetails($_GET['quote_id']);
	}
	
	if(isset($_GET['succs_msg_for_insert']))
	 {
	 	  $objSmarty->assign('SuccessMessage', 'Quote Updated Successfully');
	 } 
	 if((isset($_GET['sl_no'])) && (isset($_GET['quote_id'])))
	{
	$ObjQuoteReg->DeleteQuoteDetail($_GET['sl_no'] , $_GET['quote_id']);
	}
  ...
  $objSmarty->assign('IncludeTpl', 'quote_edit.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- QuoteReg');
