<?php
  require('includes/common.php');
	...
	$objServiceQuoteReg->ServiceRevQuote($_GET);
		if(isset($_POST['hdAction']))
	{
		$objServiceQuoteReg->ReviseService($_POST);
	}
		if(isset($_GET['service_quote_no']) && isset($_GET['revised_version']))
	{
	$objServiceQuoteReg->Revised_List(($_GET['service_quote_no']) && $_GET['revised_version']);
	$objServiceQuoteReg->Revised_Service($_GET['service_quote_no'] , $_GET['revised_version']);
	}
	if(isset($_GET['service_quote_no']))
	{
		$objServiceQuoteReg->SelectQuote(($_GET['service_quote_no']));
	}
	if(isset($_GET['succs_msg_for_insert']))
	{
		$objSmarty->assign('SuccessMessage', 'Services Updated Successfully');
	}
	 if((isset($_GET['sl_no'])) && (isset($_GET['revised_version'])) && (isset($_GET['service_quote_no'])) && (isset($_GET['total'])) && (isset($_GET['sub_total'])))
	{ 

		$objServiceQuoteReg->DeleteRev($_GET['sl_no'],$_GET['revised_version'], $_GET['service_quote_no'] , $_GET['total'] , $_GET['sub_total']);
	
	} 
if(isset($_GET['succs_msg_for_del']))
	 {
	  $objSmarty->assign('SuccessMessage', 'Revised Service Deleted Successfully');
	 }

  $objSmarty->assign('IncludeTpl', 'service_revise.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- Add Service');
