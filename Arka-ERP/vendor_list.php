<?php
  require('includes/common.php');
	...
	$ObjVendorMgmt->RolesLists($objArray);
	$ObjVendorMgmt->Listvendor();
	if(isset($_GET['vendor_id']))
	{  
		$ObjVendorMgmt->DeleteMaster($_GET['vendor_id']);
	}
	if(isset($_GET['vendor_id']) && isset($_GET['retrive']))
	{   
		$ObjVendorMgmt->RetriveMaster($_GET['vendor_id'],$_GET['retrive']);
	}
	
		if(isset($_GET['succs_msg_for_del']))
         {
          $objSmarty->assign('SuccessMessage', 'Vendor Deleted Successfully');
         }
		 if(isset($_GET['succs_msg_for_ret']))
         {
          $objSmarty->assign('SuccessMessage', 'Vendor Retrived Successfully');
         }  

  $objSmarty->assign('IncludeTpl', 'vendor_list.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- Vendor List');
