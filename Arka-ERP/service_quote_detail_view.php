<?php
  require('includes/common.php');
	...
	if(isset($_GET['service_order_no']))
	{
		$objServiceOrder->ViewServiceOrderQuote1($_GET['service_order_no']);
		$objServiceOrder->ViewServiceOrderQuote2($_GET['service_order_no']);
		$objServiceOrder->ViewServiceInvoiceQuote($_GET['service_order_no']);
		$objServiceOrder->ViewPayment($_GET['service_order_no']);
	    $objServiceOrder->followup_list($_GET['service_order_no']);

	}
	if((isset($_GET['sl_no'])) && (isset($_GET['service_order_no'])))
	{
	$objServiceOrder->DeleteOrder($_GET['sl_no'] , $_GET['service_order_no']);
	}
	 	if(isset($_GET['succs_msg_for_del']))
         {
          $objSmarty->assign('SuccessMessage', 'Service Order FollowUp Deleted Successfully');
         } 
	$objServiceOrder->DateStamp();
	$objServiceOrder->TimeStamp();
	$objServiceOrder->Created_date();

  $objSmarty->assign('IncludeTpl', 'service_quote_detail_view.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- ServiceQuoteDetail');
