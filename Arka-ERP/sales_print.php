<?php
  require('includes/common.php');
	...
	if(isset($_GET['enquiry_id']) || isset($_GET['sales_quote_no']) || isset($_GET['enquiry_type']) || isset($_GET['revised_version']))
	{
	$ObjSalesQuoteReg->SalesPrint(($_GET['enquiry_id']) && ($_GET['revised_version']));
	$ObjSalesQuoteReg->ToAddress(($_GET['sales_quote_no']) && ($_GET['revised_version'])); 
	$ObjSalesQuoteReg->Revised_Sale(($_GET['sales_quote_no']) && ($_GET['revised_version'])); 
	$ObjSalesQuoteReg->add_Total(($_GET['sales_quote_no']) && ($_GET['revised_version']));
	$ObjSalesQuoteReg->sub_Total(($_GET['sales_quote_no']) && ($_GET['revised_version']));
	$ObjSalesQuoteReg->tax_Total(($_GET['sales_quote_no']) && ($_GET['revised_version']));
	$ObjSalesQuoteReg->SalesTerms(($_GET['enquiry_type']) && ($_GET['revised_version']));
	$ObjSalesQuoteReg->Enquiry(($_GET['enquiry_id']));
	$ObjSalesQuoteReg->pro_enq_Type(($_GET['enquiry_id']) && ($_GET['revised_version']));
	}


  $objSmarty->assign('IncludeTpl', 'sales_print.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- Sales Quote');
