<?php
  require('includes/common.php');
	...
	$ObjEnquiryReg->ViewEnquiry();
	if(isset($_GET['searchvalue1']))
	{
		$ObjEnquiryReg->ViewEnquiryStatus();
	}
	if(isset($_GET['enquiry_id']))
	{
		$ObjEnquiryReg->DeleteMaster($_GET['enquiry_id']);
	}
		if(isset($_GET['succs_msg_for_del']))
         {
          $objSmarty->assign('SuccessMessage', 'Enquiry Deleted Successfully');
         } 
  $objSmarty->assign('IncludeTpl', 'enquiry_list.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- Enquiry List');
