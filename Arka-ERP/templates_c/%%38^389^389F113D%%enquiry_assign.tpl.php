<?php /* Smarty version 2.6.25, created on 2016-10-14 10:21:12
         compiled from enquiry_assign.tpl */ ?>
<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
...
<thead>
	...
	<tbody>
		<?php $_from = $this->_tpl_vars['ListUser']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }if (count($_from)):
    foreach ($_from as $this->_tpl_vars['ListDet']):
?>
			<tr>
				<td><input type="checkbox" name="checkbox" id="checkbox[<?php echo $this->_tpl_vars['ListDet']['username']; ?>
]"  onClick="closeit('<?php echo $this->_tpl_vars['ListDet']['username']; ?>
');"></td>

				<td><?php echo $this->_tpl_vars['ListDet']['username']; ?>
</td>
			</tr>
...
<script>
function Remove_Assign_To(childelemnt)
{
var child = parent.document.getElementById(childelemnt);
alert(child);exit;
var parent_div = parent.document.getElementById("standard_bar1");
...
function closeit(val)
{
	...
	{
	var std_bar_obj=parent.document.getElementById("standard_bar1");
	var btn = parent.document.createElement("DIV");
	btn.setAttribute("id", val);
	btn.innerHTML=val+",";
	btn.setAttribute("class", "selected_prdts");
	btn.setAttribute("onClick", "Remove_Assign_To(this.id)");
