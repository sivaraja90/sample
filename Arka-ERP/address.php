...
<?php
  require('includes/common.php');
	...
	$ObjPoReg	= new PoReg();
	//$ObjPoReg	->ViewEnquirymaster($objArray);
      //$ObjPoReg	->GetEnquiryType($objArray);
	if(isset($_POST['hdAction']))
	{
	$ObjPoReg->InsertAddress($_POST);
	}
	if(isset($_GET['succs_msg_for_insert']))
         {
          $objSmarty->assign('SuccessMessage', 'New Address Added Successfully');
         } 
//	$ObjDashboard->RolesLists();
	// $ObjDashboard->myCurl($objArray);

  $objSmarty->assign('IncludeTpl', 'address.tpl');
