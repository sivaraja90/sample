<?php
  require('includes/common.php');
	...
	include_once $config['SiteClassPath']."class.MaterialReg.php";
	...
	$ObjMaterialReg= new MaterialReg();
if(isset($_GET['server_response']))
{
	$objSmarty->assign('SuccessMessage', 'Update Material and Entitled Added Successfully');
} 
if(isset($_POST['hdAction']))
{
	$ObjMaterialReg->EditMaterial($_POST);
}
if(isset($_GET['material_code']))
{
	$ObjMaterialReg->ViewEdit($_GET['material_code']);
}
if(isset($_GET['succs_msg_for_insert']))
{
	$objSmarty->assign('SuccessMessage', 'Material Updated Successfully');
}
	
	$objSmarty->assign('IncludeTpl', 'material_edit.tpl');
	$objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- MaterialReg');
