<?php
  require('includes/common.php');
	...
	if(isset($_GET['service_quote_revise_no']))
	{
	$ObjServiceOrder->mail_list($_GET['service_quote_revise_no']);
	}
		if(isset($_POST['email_id1']))
	{
		$ObjServiceOrder->sendMail($_POST['email_id1']);
    }
		if(isset($_POST['hdAction']))
	{
	$ObjServiceOrder->InsertServiceOrderMail($_POST,$_FILES);
	}

if(isset($_GET['succs_msg']))
 {
	$objSmarty->assign('SuccessMessage', 'Email Send Successfully');
 } 
  $objSmarty->assign('IncludeTpl', 'service_order_mail.tpl');
