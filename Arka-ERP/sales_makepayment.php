...
<?php
  require('includes/common.php');
	...
	$ObjSalesOrder->ViewPaymentInvoice();
		if(isset($_GET['sales_order_no']))
	{
	$ObjSalesOrder	->ToAddress2($_GET["sales_order_no"]);
	}
     $ObjSalesOrder	->GetEnquiryType($objArray);
     $ObjSalesOrder->GetSalesPayment();
	if(isset($_POST['hdAction']))
	{
	$ObjSalesOrder->InsertSalesMakepayment($_POST);
	}
	if(isset($_GET['succs_msg_for_insert']))
         {
          $objSmarty->assign('SuccessMessage', 'New Payment Added Successfully');
         } 
  $objSmarty->assign('IncludeTpl', 'sales_makepayment.tpl');
