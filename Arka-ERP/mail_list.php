<?php
  require('includes/common.php');
	...
	include_once $config['SiteClassPath']."class.MailMgmt.php";
	...
	$ObjMailMgmt	= new MailMgmt();
	$ObjMailMgmt-> EnquiryMail();
	$ObjMailMgmt-> SalOrderMail();
	$ObjMailMgmt-> ServOrderMail();
	$ObjMailMgmt-> ProjOrderMail();

 	if(isset($_GET['sl_no']) && isset($_GET['enquiry_id']))
	{ 
  $ObjMailMgmt->DeleteEnquiryMail($id);
  	}
	if(isset($_GET['sl_no']) && isset($_GET['enquiry_id']))
	{ 
  $ObjMailMgmt->DeleteSalesMail($id);
  	}
	if(isset($_GET['sl_no']) && isset($_GET['enquiry_id']))
	{ 
  $ObjMailMgmt->DeleteServiceMail($id);
    }
	if(isset($_GET['sl_no']) && isset($_GET['enquiry_id']))
	{ 
  $ObjMailMgmt->DeleteProjectMail($id);
    }
	 if(isset($_GET['succs_msg_for_insert']))
       {
          $objSmarty->assign('SuccessMessage', 'Mail Deleted Successfully');
       }  
  $objSmarty->assign('IncludeTpl', 'mail_list.tpl');
  $objSmarty->assign('OverallSiteTitle', $config['SiteTitle'].'- MailList');
